...
/**
 * Single item model for a customer
 *
 * @package     Joomla.Site
 * @subpackage  com_vmmdatabase
...
class CategoryModel extends ListModel
{
	...
	/**
	 * Array of vmmdatabase in the category
	 *
	 * @var    \stdClass[]
	 */
	...
	/**
	 * The list of other customer categories.
	 *
	 * @var    array
	 */
	...
	public function getItems()
	{
		...
		for ($i = 0, $n = count($items); $i < $n; $i++) {
			...
			if ($this->getState('load_tags', true)) {
				...
				$this->tags->getItemTags('com_vmmdatabase.customer', $item->id);
			}
		}
	...
	protected function getListQuery()
	{
		...
		$query->select($this->getState('list.select', 'a.*'))
			...
			->from($db->quoteName('#__vmmdatabase_details', 'a'))
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$numberOfVmmdatabaseToDisplay = $mergedParams->get('vmmdatabase_display_num');

		if ($format === 'feed') {
			$limit = $app->get('feed_limit');
		} else if (isset($numberOfVmmdatabaseToDisplay)) {
			$limit = $numberOfVmmdatabaseToDisplay;
		} else {
		...
		$search = $app->getUserStateFromRequest('com_vmmdatabase.category.list.' . $itemid . '.filter-search', 'filter-search', '', 'string');
		...
		if ((!$user->authorise('core.edit.state', 'com_vmmdatabase')) && (!$user->authorise('core.edit', 'com_vmmdatabase'))) {
	...
	public function getCategory()
	{
		if (!is_object($this->_item)) {
			...
			$categories = Categories::getInstance('Vmmdatabase', $options);
