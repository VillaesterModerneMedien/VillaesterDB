...
class AdministratorService
{
	 ...
	 *
	 * @param   integer  $customerid  The item id to search associations
	 *
	 * @return  string  The language HTML
	 *
	 * @throws  Exception
	 */
	public function association($customerid)
	{
		// Defaults
		$html = '';

		// Get the associations
		if ($associations = Associations::getAssociations('com_vmmdatabase', '#__vmmdatabase_details', 'com_vmmdatabase.item', $customerid, 'id', null)) {
			foreach ($associations as $tag => $associated) {
				$associations[$tag] = (int) $associated->id;
			}

			// Get the associated customer items
			$db = Factory::getDbo();
			$query = $db->getQuery(true)
				->select('c.id, c.name as title')
				->select('l.sef as lang_sef, lang_code')
				->from('#__vmmdatabase_details as c')
			...
			if ($items) {
				foreach ($items as &$item) {
					...
					$url = Route::_('index.php?option=com_vmmdatabase&task=customer.edit&id=' . (int) $item->id);
	 ...
	 *
	 * @return  string	The anchor tag to toggle featured/unfeatured vmmdatabase.
	 *
	 * @since   __BUMP_VERSION__
	 */
	public function featured($value, $i, $canChange = true)
	{
		...
		$states = [
			0 => ['unfeatured', 'vmmdatabase.featured', 'COM_CONTACT_UNFEATURED', 'JGLOBAL_ITEM_FEATURE'],
			1 => ['featured', 'vmmdatabase.unfeatured', 'JFEATURED', 'JGLOBAL_ITEM_UNFEATURE'],
		];
